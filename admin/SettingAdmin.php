  ...
  if (isset($_POST['update_info'])) {
    ...
    if (empty($errors['info'])) {
      try {
        ...
        $check_sql = "SELECT id FROM admin_info WHERE user_id = ?";
        ...
        if (mysqli_stmt_num_rows($check_stmt) > 0) {
          ...
          $query = "INSERT INTO admin_info (
                    last_name, first_name, middle_initial, age, contact, address, gender, birth_date, user_id
                  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        }
        ...
        <form action="" method="POST">
          ...
          <div class="personal-info-grid">
            ...
            <div>
              ...
              <input type="text" id="middle_initial" name="middle_initial" placeholder="M.I" maxlength="1"
                class="custom-input <?php echo isset($errors['info']['middle_initial']) ? 'error-field' : ''; ?>"
                value="<?php echo htmlspecialchars($admin_info_data['middle_initial'] ?? ''); ?>">
              <?php if (isset($errors['info']['middle_initial'])): ?>
                <span class="error"><?php echo htmlspecialchars($errors['info']['middle_initial']); ?></span>
              <?php endif; ?>
            </div>
